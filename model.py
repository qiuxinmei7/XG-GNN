...
# This module's implementation is partially based on the following paper:
# Kan X, Cui H, Lukemire J, et al. Fbnetgen: Task-aware GNN-based fMRI analysis via functional brain network generation[C]//International Conference on Medical Imaging with Deep Learning. PMLR, 2022: 618-637.

class ConvKRegion(nn.Module):

    def __init__(self, k=1, out_size=8, kernel_size=8, pool_size=16, time_series=100):
        super().__init__()
...
class XG_GNN(nn.Module):

    def __init__(self, model_config, roi_num=200, node_feature_dim=200, time_series=100):
        super().__init__()
        self.extract = ConvKRegion(out_size=model_config['embedding_size'], kernel_size=model_config['window_size'], time_series=time_series)
        self.learn_fc = MHSA(model_config['embedding_size'], roi_num=roi_num)
        self.predictor = GNNDIAGNOSER(node_feature_dim, roi_num=roi_num)
